...
class DevicePublish {
    ...
    getDevice(deviceID) {
        ...
        const ieeeAddr = settings.getIeeeAddrByFriendlyName(deviceID) || deviceID;

        // Get device
        const device = this.zigbee.getDevice(ieeeAddr);

        return {ieeeAddr, device};
    }
    ...
    parseTopic(topic) {
        ...
        // Try to get device
        let deviceInfo = this.getDevice(topic);

        while (!deviceInfo.device && topic.includes('/')) {
            key = topic.substr(topic.lastIndexOf('/') + 1, topic.length) + key;

            // Remove key from topic
            topic = topic.substr(0, topic.lastIndexOf('/'));

            // Try to get device
            deviceInfo = this.getDevice(topic);
        }

        if (!deviceInfo.device) {
            logger.error(`Failed to find device for topic substring: '${key}'`);
            return;
        }

        return {type: type, key: key, deviceID: topic, ieeeAddr: deviceInfo.ieeeAddr, device: deviceInfo.device, postfix: postfix};
    }
    ...
    onMQTTMessage(topic, message) {
        ...
        const model = zigbeeShepherdConverters.findByZigbeeModel(topic.device.modelId);
        if (!model) {
            logger.warn(`Device with modelID '${topic.device.modelId}' is not supported.`);
        ...
        if (model.hasOwnProperty('ep')) {
            const eps = model.ep(topic.device);
        ...
        Object.keys(json).forEach((key) => {
                ...
                this.zigbee.publish(
                    ...
                    (error, rsp) => {
                        ...
                        if (topic.type === 'set' && !error && (key.startsWith('state') || key === 'brightness')) {
                            ...
                            this.publishDeviceState(topic.device, msg, true);
                        }
            ...
            if (topic.type === 'set' && converted.zclData.transtime) {
                ...
                setTimeout(() => {
                    ...
                    this.queue.push((queueCallback) => {
                        this.zigbee.publish(
                            topic.ieeeAddr, getConverted.cid, getConverted.cmd, getConverted.cmdType,
